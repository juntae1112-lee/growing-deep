...
> 동시성 문제를 해결할 때 lock은 강력한 선택지지만, 단일 상태 변경에는 atomic operation이라는 다른 경계 설정 방식도 있다.
...
처음에는 동시성 문제를 도구 선택의 문제로 봤다.

어떤 lock을 써야 하는지, 어디까지 보호해야 하는지, 어떤 동기화 객체가 적절한지가 먼저 떠올랐다.

하지만 점점 중요한 질문은 도구 자체보다 "무엇을 공유하고 있고, 어떤 단위의 변경을 보호하려는가"에 가깝다고 느끼게 되었다.
