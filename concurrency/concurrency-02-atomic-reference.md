...
> atomic은 숫자를 안전하게 증가시키는 기술을 넘어, 완성된 새 상태를 만든 뒤 참조를 교체하는 방식으로도 이해할 수 있다.
...
이때 내 관심은 주로 연산 하나가 깨지지 않는가에 있었다.

하지만 시간이 지나면서 관심이 조금 바뀌었다.

하나의 값을 안전하게 바꾸는 것보다, 상태를 어떤 단위로 만들고 어떤 방식으로 교체할 것인가가 더 중요하게 보이기 시작했다.
