...
처음에는 `StateFlow`가 atomic하게 동작한다는 사실이 가장 중요해 보였다.

하지만 다시 보니 더 중요한 것은 현재 구조에서 누가 state를 쓰고 있는가였다.

상태 객체가 immutable인지, 참조 교체가 atomic인지도 중요하지만, 결국 여러 writer가 같은 상태를 동시에 바꾸려는 구조인지부터 봐야 했다.
